...
// ==========================
const calculatedfields = fieldList.filter(item => item.calculatedFieldKey);
console.log("Calculated Fields:", calculatedfields);
// ==========================
// STEP 4: Remove Discount
// ==========================
const index = fieldList.findIndex(item => item.entityName === "Discount");
if (index !== -1) {
...
const numericalVariables = allVariables.filter(v => v.variableType === "numerical");
...
// ==========================
fields.Measures = fields.Measures.map((item) => {
    return {
        ...item,
...
const NumericalItems = fieldList
    .filter(item => item.isNumericDataType === true &&
    item.dataType === "float")
    .map(item => ({
    ...item,
